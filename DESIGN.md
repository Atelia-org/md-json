...
| U6 | 用户后续决定 | 显式路径列表改为候选集合；仅当字符串按骨架的 JSON 编码器写入时需要转义，才外置为正文。 |
...
仅三个概念：JSON 骨架、候选外置 JSON Pointer 列表、带路径的字符串正文块。路径同时充当地址和标识，无独立 ID、引用图或内容去重。
...
- `externalStringPaths` 是精确的候选路径列表；空列表表示所有值保留在 JSON 中。列表里的路径仍须全部有效，但只有其字符串值在当前骨架编码器下需要 JSON 转义时才外置。
...
- 写入前验证所有路径：路径重复、不存在、语法错误或目标不是字符串，都失败，不静默跳过。校验通过后才判断该值是否需要外置。
...
实际外置的正文按 `externalStringPaths` 的传入顺序输出，不再执行另一套排序。需要与骨架顺序一致时，调用方按该顺序传入列表。骨架保留输入对象属性顺序与数组顺序；读回结果不依赖正文块的排列。

## 4. 文档形状

固定两个顶层区段，顺序为 `# structure`、`# content`。structure 内恰好一个 `json` 围栏；content 内为零个或多个“路径标题 + text 围栏”。空路径列表仍输出空的 content 区段。

本例渲染参数依次为 `/observation/0/intention`、`/recall/0/content`。两处字符串都含换行，需要 JSON 转义，因此均外置。原正文均不含末尾 LF，闭围栏直接位于正文最后一行的下一行；不额外插入空白行。精确字符规则见 §6。
...
写入规则：对每个候选位置 `p`，先取解码后的字符串，用与骨架相同的 JSON 编码器写成字符串字面量。如果除两侧引号外的编码结果与原字符串逐字符相同，就在骨架中保留原值，不产生正文块；否则把骨架中的值替换为字符串 `p`，将原字符串写到标题为 `p` 的正文块中。不能依据原始 JSON 的转义拼写或只搜索原字符串中的反斜杠来决定：例如源文本 `"\\u0061"` 解码后是 `a`，可以留在骨架；实际换行和反斜杠都需要转义。相同正文出现在两个候选位置且均需转义时，分别输出两个块。

此规则只判断 JSON 转义需求；很长但无需转义的单行字符串仍保留在骨架中，不另外引入长度阈值。
...
原值 `{"x":"/x"}` 配合空正文区段，是合法文档。把原值 `{"x":"任意\n正文"}` 外置后，再完整删除 `/x` 的正文标题和围栏，也可能形成同一个合法文档。因此：
...
1. 用户的 Observation + Recall 示例；传入全部、部分和零个候选路径，验证只生成需要转义的正文。
...
8. 候选字符串中的普通文本和空串留在骨架；实际换行、引号、反斜杠及编码器要求转义的 Unicode 字符外置。原始 JSON 写作 `"\\u0061"` 时按解码后的 `a` 判断；豁免路径仍接受全部路径校验。

以上验收类别已落实为 xUnit 测试，其中一个确定性生成测试包含 128 组字符串。首版的独立 PackageReference 消费者曾通过公开接口验证；后续规则由当前测试覆盖。LLM 阅读关联准确率和 token 数仍需另用真实样本比较，不作为已证实收益。
